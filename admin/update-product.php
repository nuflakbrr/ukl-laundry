            ...
            <div class="relative py-3 sm:max-w-xl sm:mx-auto">
                ...
                <div class="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
                    <div class="max-w-md mx-auto">
                        <div>
                            <h1 class="text-2xl font-semibold">Silakan Masukkan Data Produk!</h1>
                        </div>
                        <div class="divide-y divide-gray-200">
                            <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                                <?php
                                    include ('../sql/db-laundry.php');
                                    $qry_get=mysqli_query($con,"select * from package where type = '".$_GET['type']."'");
                                    $dt_get=mysqli_fetch_array($qry_get);
                                ?>
                                <form action="../utils/process-update-product.php" method="post">
                                    <div class="relative mt-5">
                                        ...
                                        <select name="type" id="type" class="peer placeholder-transparent h-10 w-full border-gray-300 text-gray-900 focus:outline-none">
                                            ...
                                            <?php foreach ($arr_type as $key_type => $val_type):
                                              if($key_type==$dt_get['type']){
                                                  $selek="selected";
                                              } else {
                                                  $selek="";
                                              }
                                            ?>
                                              <option value="<?=$key_type?>" <?=$selek?>><?=$val_type?></option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                                    <div class="relative mt-5">
                                        <input autocomplete="off" id="price" name="price" type="text" class="peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 focus:outline-none" placeholder="Harga" value="<?=$dt_get['price'] ?>" />
